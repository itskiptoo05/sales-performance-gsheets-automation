...
The goal is to demonstrate how small to medium-sized businesses (like a Kenyan tech/computer shop) can automate performance tracking, reporting, and decision-making using tools already available in Google Workspace.
...
This script enriches the sales data in the 'Processed' sheet by adding:
...
After enriching the dataset using Apps Script, the `Dashboard` sheet visualizes key metrics and trends derived from the `Processed` sheet.
...
- **Bar Chart**: Clear comparison of unit sales across categories
...
- **Bar chart**: Used to show total sales volume per person
...
This leaderboard combines both revenue performance and transactional volume to rank sales reps more fairly and holistically.
...
- **Bar Chart**: Highlights the absolute performance of each rep.
...
- **Bar Chart**: Compares absolute sales per model.
